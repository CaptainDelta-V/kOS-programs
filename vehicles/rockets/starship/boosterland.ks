...
LOCAL RADAR_OFFSET TO 65.5.
LOCAL TOWER_CATCH_ALT TO 139.5.
...
RCS ON.
LOCK STEERING TO HEADING(LANDING_STATUS:RETROGRADE_HEADING(), BOOSTBACK_PITCH, 180). 
...
WAIT UNTIL ALTITUDE < 80_000.
    ...
    SET LANDING_STATUS TO LANDING_STATUS_MODEL(LANDING_SITE, VIRTUAL_TARGET_ALTITUDE):OVERSHOOT(-40).    
...
WAIT UNTIL ALTITUDE < 70_000.   
    ...
    LOCK STEERING TO LANDING_STEERING:STEERING_VECTOR(TARGET_ROLL).
...
WAIT UNTIL ALTITUDE < 25_000.     
    LANDING_STATUS:SET_LANDING_SITE(LANDING_STATUS_MODEL(LANDING_SITE):OVERSHOOT(-20):GET_LANDING_SITE()).     
...
LOCAL LANDING_BURN_START TO FALSE. 
UNTIL LANDING_BURN_START { 

    IF LANDING_BURN:TRUE_RADAR() < LANDING_BURN:STOP_DISTANCE() + SUICIDE_MARGIN { 
        SET LANDING_BURN_START TO TRUE.
...
UNTIL HOVER_START {         
    SET HOVER_START TO SHIP:VERTICALSPEED > -0.5 OR LANDING_BURN:TRUE_RADAR() < 0.              
    WAIT 0.                
}
