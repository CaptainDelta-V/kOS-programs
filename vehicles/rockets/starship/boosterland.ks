...
RCS ON.
LOCK STEERING TO HEADING(LANDING_STATUS:RETROGRADE_HEADING(), BOOSTBACK_PITCH, 0). 
...
LANDING_BURN:SET_RADAR_OFFSET(1_000).
LOCAL SUICIDE_MARGIN TO 500.
WHEN LANDING_BURN:TRUE_RADAR() < LANDING_BURN:STOP_DISTANCE() + SUICIDE_MARGIN THEN {
    ...
    RUN_ALTITUDE_HOLD(TOWER_CATCH_ALT, 4).            
    LOCK THROTTLE TO LANDING_BURN:IDEAL_THROTTLE().  
